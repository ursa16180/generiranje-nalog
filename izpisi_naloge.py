...
def sestaviVseTeste(datoteka_seznam_dijakov, naloge, ime_naloge=date.today().strftime("%d-%B-%Y"),
                    zdruzene_resitve=True):  ###TODO če ni seznama naredi 1 test
...
def napisiTest(vzorec_testa, vzorec_resitev, naloge, dijak, potNaloge,
               potResitve):  # Napiše naloge in njihove rešitve #TODO rešitve kot txt?
...
def napisiVzorecTesta(steviloNalog, podmapa):  # TODO grafi
    with open("%s/vzorec_testa.txt" % podmapa, "w+", encoding="utf8") as vzorec_testa:
        vzorec_testa.writelines(
